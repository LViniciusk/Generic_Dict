...
private:
    ...
    Node<T>* delete_successor(Node<T>* node, Node<T>* successor) {
        if (successor->left != nullptr) successor->left = delete_successor(node, successor->left);
        else {
    ...
    Node<T>* _delete(Node<T>* node, T key, unsigned int value = 1) {
        ...
        if (key < node->key) node->left = _delete(node->left, key, value);
        else if (key > node->key) node->right = _delete(node->right, key, value);
        ...
        else node->right = delete_successor(node, node->right);
    ...
    Node<T>* _update(Node<T>* node, T key, unsigned int value) {
        if (node == nullptr) return node;
        if (compare(key, node->key)) node->left = _update(node->left, key, value);
        else if (compare(node->key, key)) node->right = _update(node->right, key, value);
        else node->frequence = value;
        return node;
    }
